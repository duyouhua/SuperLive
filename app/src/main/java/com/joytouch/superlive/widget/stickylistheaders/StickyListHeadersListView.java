...
import com.joytouch.superlive.widget.stickylistheaders.WrapperViewList.LifeCycleListener;
...
public class StickyListHeadersListView extends FrameLayout implements Pullable {
    ...
    public boolean canPullDown()
    {
        ...
        if (getCount() == 0)
        {
            // 没有item的时候也可以下拉刷新
            return true;
        } else if (mList!=null&&mList.getFirstVisiblePosition() == 0
                &&mList.getChildAt(0)!=null&&mList.getChildAt(0).getTop() >= 0)
        {
            // 滑到ListView的顶部了
            return true;
        } else
    ...
    public boolean canPullUp()
    {
        if (mList.getCount() == 0)
        {
            // 没有item的时候也可以上拉加载
            return true;
        } else if (mList.getLastVisiblePosition() == (mList.getCount() - 1))
        {
            // 滑到底部了
            if (mList.getChildAt(getLastVisiblePosition() - mList.getFirstVisiblePosition()) != null
                    && mList.getChildAt(
                    mList.getLastVisiblePosition()
                            - mList.getFirstVisiblePosition()).getBottom() <= mList.getMeasuredHeight())
                return true;
        }
        return false;
    }
