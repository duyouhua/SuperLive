...
public class CustomDigitalClock extends DigitalClock {
		...
		mTicker = new Runnable() {
			public void run() {
				...
				if (distanceTime == 0) {
					...
					LogUtils.e("------------------",dealTime(distanceTime));
	...
	public static String dealTime(long time) {
		...
		long mm = ((time % (24 * 60 * 60)) % (60 * 60)) % 60/10%100;
		String dayStr = String.valueOf(day);
		String hoursStr = timeStrFormat(String.valueOf(hours));
		String minutesStr = timeStrFormat(String.valueOf(minutes));
		String secondStr = timeStrFormat(String.valueOf(second));
		String mmStr = timeStrFormat(String.valueOf(mm));
		returnString.append(secondStr).append("″").append(mmStr);
