...
import com.joytouch.superlive.javabean.Cityinfo;
...
public class CityPicker extends LinearLayout {
	...
	protected void onFinishInflate() {
		...
		provincePicker.setData(citycodeUtil.getProvince(province_list),citycodeUtil.getProvince(province_list));
		provincePicker.setDefault(1);
		cityPicker.setData(citycodeUtil.getCity(city_map, citycodeUtil
				.getProvince_list_code().get(1)),citycodeUtil.getProvince(province_list));
		cityPicker.setDefault(1);
		counyPicker.setData(citycodeUtil.getCouny(couny_map, citycodeUtil
				.getCity_list_code().get(1)),citycodeUtil.getProvince(province_list));
			...
			public void endSelect(int id, String text,String areaid) {
				...
				if (tempProvinceIndex != id) {
					...
					cityPicker.setData(citycodeUtil.getCity(city_map,
							citycodeUtil.getProvince_list_code().get(id)),citycodeUtil.getProvince(province_list));
					cityPicker.setDefault(1);
					counyPicker.setData(citycodeUtil.getCouny(couny_map,
							citycodeUtil.getCity_list_code().get(1)),citycodeUtil.getProvince(province_list));
					counyPicker.setDefault(1);
			...
			public void endSelect(int id, String text,String areaid) {
				...
				if (temCityIndex != id) {
					...
					if (selectMonth == null || selectMonth.equals(""))
						return;
					counyPicker.setData(citycodeUtil.getCouny(couny_map,
							citycodeUtil.getCity_list_code().get(id)),citycodeUtil.getProvince(province_list));
					counyPicker.setDefault(1);
