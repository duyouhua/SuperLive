...
public class ScrollerNumberPicker extends View {
	...
	/**设置携带的地区id*/
	private ArrayList<String>  AreaidList= new ArrayList<String>();
	/** 按下的坐标 */
	private int downY;
	/** 按下的时间 */
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				if (onSelectListener != null)
					onSelectListener.endSelect(item.id, item.itemText,item.areaid);
				break;
			}
		}
	}
	...
	private void initData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			itmItemObject.x = 0;
			itmItemObject.areaid=AreaidList.get(i);
				...
				if (itemList.get(i).isSelected()) {
					...
					if (onSelectListener != null)
						onSelectListener.endSelect(itemList.get(i).id,
								itemList.get(i).itemText,itemList.get(i).areaid);
					break;
				}
			}
		} else {
	 ...
	 * @param data
	 */
	public void setData(ArrayList<String> data,ArrayList<String> AreaidList) {
		this.dataList = data;
